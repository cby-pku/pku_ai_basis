...
#网页可视化
fig = plt.figure(figsize=(8,8))#创建一个figure对象，并设置对象大小为8*8
plt.axis("off")#关闭坐标轴，更加美观简洁
#列表推导式将图像列表 img_list 转换为一个三重嵌套列表 ims，其中每个子列表中有一个 imshow 对象，该对象包含一个在 matplotlib 中显示图像的函数 imshow()，
# 以及用于将图像转置为HWC 维度顺序的 numpy 函数的np.transpose()
ims = [[plt.imshow(np.transpose(i,(1,2,0)), animated=True)] for i in img_list]
#创建一个ani对象来生成动画，第一个参数使前面创建的figure对象，第二个参数是三重嵌套列表ims，第三个参数用于控制每个图像之间转换的时间见个和切换速度
